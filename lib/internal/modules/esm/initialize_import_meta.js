...
function createImportMetaLoad(defaultParentURL, loader, allowParentURL) {
  /**
   * @param {string} url
   * @param {URL['href']} [parentURL] When `--experimental-import-meta-resolve` is specified, a
   * second argument can be provided.
   */
  return function load(url, parentURL = defaultParentURL) {
    if (!allowParentURL) { parentURL = defaultParentURL; }

    return loader.loadSync(url, { parentURL }).source + '';
  };
}
...
function initializeImportMeta(meta, context, loader) {
  ...
  if (!loader || loader.allowImportMetaResolve) {
    ...
    meta.load = createImportMetaLoad(url, loader, experimentalImportMetaResolve);
  }
