...
class ESMLoader {
  ...
  async addCustomLoaders(
    ...
    for (let i = 0; i < customLoaders.length; i++) {
      ...
      if (globalPreloader) {
        const hook = ObjectCreate(null);
        hook.url = url;
        hook.fn = FunctionPrototypeBind(globalPreloader); // [1]

        this.#globalPreloaders.push(hook);
      }
      if (resolver) {
        const hook = ObjectCreate(null);
        hook.url = url;
        hook.fn = FunctionPrototypeBind(resolver); // [1]

        this.#resolvers.push(hook);
      }
      if (loader) {
        const hook = ObjectCreate(null);
        hook.url = url;
        hook.fn = FunctionPrototypeBind(loader); // [1]

        this.#loaders.push(hook);
      }
    }
