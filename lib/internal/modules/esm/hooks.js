...
class Hooks {
  #chains = {
    ...
    resolve: new Chain('resolve', {
      validateArgs: (hookErrIdentifier, suppliedSpecifier, ctx) => {
        validateString(
          suppliedSpecifier,
...
/**
 * @typedef {'load' | 'initialize' | 'resolve'} HookType Which kind of hook this is.
 */
...
class Chain {
   ...
   */
  append(type, validateArgs, { fn, url }) {
    const newHook = new Hook(fn, { type, url }, validateArgs);
    newHook.next = this.#end;
    this.#end.prev = newHook;

    return (this.#end = newHook);
  }
}
 ...
 * @prop {import('./loader.js').ModuleFormat} [format] Hint to the load hook (it might be ignored).
 * @prop {object} [importAttributes] The import attributes to use when caching the module (if
 ...
 * @prop {import('./loader.js').ModuleFormat} format The format of the included source.
 * @prop {boolean} [shortCircuit=false] A signal that this hook intends to terminate the chain.
 ...
 * @param {boolean} HookRunMeta.chainFinished Whether the end of the chain has been reached AND
 * invoked.
 * @param {boolean} HookRunMeta.shortCircuited Whether a hook signaled a short-circuit.
 */
...
class Hook {
  ...
  /**
   * @type {HookValidators['validateArgs']}
   */
  #validateArgs;
  ...
  /**
   * @type {'nextLoad' | 'nextInitialize' | 'nextResolve'}
   */
  get name() {
    return `next${
   ...
   * @returns {this['type'] extends 'load' ? LoadHookOutput : ResolveHookOutput} The validated
   * output from the hook.
   */
  async run(arg0, context, meta) {
    this.#validateArgs(this.prev.errIdentifier, arg0, context);
