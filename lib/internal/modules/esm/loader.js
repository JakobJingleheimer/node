...
function debug(...args) {
  require('fs').appendFileSync('/dev/fd/1',
    args
      .map((arg) => require('util').inspect(arg))
      .join(' ')
    + '\n'
  );
}
...
debug('BaseESMLoader::#createModuleJob()', this.constructor.name);
    const moduleProvider = async (url, isMain) => {
      ...
      const translator = this.translators.get(finalFormat);
...
debug('BaseESMLoader::getModuleJob() this', this.constructor.name)
debug('BaseESMLoader::getModuleJob() args', { url, parentURL, importAssertions, format })
    const job = (
      this.moduleMap.get(url, importAssertions.type) ??
      this.#createModuleJob(url, importAssertions, parentURL, format)
    );
debug('BaseESMLoader::getModuleJob() job', job)
...
debug('BaseESMLoader::import() jobs', jobs);
    for (let i = 0; i < count; i++) {
      const job = this.runModuleJob(specifiers[i], parentURL, importAssertions)
debug('BaseESMLoader::import() ran job', jobs);
      jobs[i] = job.then(({ module }) => module.getNamespace());
debug('BaseESMLoader::import() got namespace (unsettled)', jobs[i]);
    }

    const namespaces = await SafePromiseAllReturnArrayLike(jobs);
debug('BaseESMLoader::import() namespaces settled', namespaces);
...
// ObjectSetPrototypeOf(BaseESMLoader.prototype, null); // ! this breaks `super`
...
debug('DefaultESMLoader::runModuleJob()', { specifier, parentURL, importAssertions });
  ...
  importMetaInitialize(meta, context) {
    return this.#hooks.importMetaInitialize(meta, context);
  }
...
debug('CustomisedESMLoader::getModuleJob()')
    const { format, url } = await this.resolve(specifier, parentURL);

debug('CustomisedESMLoader::getModuleJob() resolved url', url);
debug('CustomisedESMLoader::getModuleJob() moduleMap', this.moduleMap);
    return ReflectApply(
      // Bypass DefaultESMLoader::getxModuleJob() to get to BaseESMLoader::getModuleJob()
      super.__proto__.__proto__.__proto__.getModuleJob,
      this,
      [url, parentURL, importAssertions, format],
    );
  }

  runModuleJob(specifier, parentURL, importAssertions) {
debug('CustomisedESMLoader::runModuleJob()');
    return this
      .getModuleJob(specifier, parentURL, importAssertions)
      .then((job) => job.run());
  }
...
class ProxiedESMLoader extends BaseESMLoader {
   ...
   */
   getModuleJob(specifier, parentURL, importAssertions) {
debug('ProxiedESMLoader::getModuleJob()')
    const { format, url } = this.resolve(specifier, parentURL);

debug('ProxiedESMLoader::getModuleJob() resolved url', url);
    return super.getModuleJob(url, parentURL, importAssertions, format);
  }

  importMetaInitialize(meta, context) {
    return this.#hooks.makeRequest('importMetaInitialize', meta, context);
  }
  ...
  runModuleJob(specifier, parentURL, importAssertions) {
debug('ProxiedESMLoader::runModuleJob()');
    return this
      .getModuleJob(specifier, parentURL, importAssertions)
      .run();
  }
...
debug('ProxiedESMLoader::resolve()')
