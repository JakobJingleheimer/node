...
class Hooks {
   ...
   * @returns {{ format: string, url: URL['href'] } | Promise<{ format: string, url: URL['href'] }>}
   */
  resolve(
    originalSpecifier,
    parentURL,
...
console.log('Hooks::resolve()', { originalSpecifier, parentURL, importAssertions })
    ...
    const finalizeResolution = (resolution) => {
...
console.log('Hooks::resolve() resolution', resolution)
    if (typeof resolution?.then === 'function') {
console.log('Hooks:resolve() resolution is thenable; thenning');
...
console.log('HooksProxy::makeRequest()', type, ...args)
    if (!this.#isReady) {
console.log('HooksProxy::makeRequest() checking if worker is ready')
      const { kIsOnline } = require('internal/worker');
      if (!this.worker[kIsOnline]) {
console.log('HooksProxy::makeRequest() worker is NOT ready. sleeping until signaled');
        Atomics.wait(this.#lock, 0, 0); // ! Block the main thread until the worker is ready.
      }

      this.#isReady = true;
console.log('HooksProxy::makeRequest() awoken: worker is now ready');
    }

    const { serialize } = require('v8');
console.log('HooksProxy::makeRequest() emptying data vehicle');
...
console.log('HooksProxy::makeRequest() sent request to worker. sleeping until notified');
    Atomics.wait(this.#lock, 0, 0); // Sleep until worker responds
console.log('HooksProxy::makeRequest() awoken: received response from worker');
    const { deserialize } = require('v8');

    const response = deserialize(this.#requestResponseData);

console.log('HooksProxy::makeRequest() response', type, '=>', response)
...
function nextHookFactory(chain, meta, { validateArgs, validateOutput }) {
  ...
  return ObjectDefineProperty(
    (arg0 = undefined, context) => {
      ...
      const output = hook(arg0, meta.context, nextNextHook);
