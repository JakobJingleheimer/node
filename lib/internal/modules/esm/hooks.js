...
class Hooks {
  ...
  async resolve(
    ...
    writeFileSync(1, `firstResolve: ${nextResolve.toString()}\n`)
    ...
    const hookErrIdentifier = nextResolve.errIdentifier; // Retrieve the value after all settled
  ...
  async load(url, context = {}) {
    ...
    while (nextLoad) {
      resolution = await nextLoad.run(originalSpecifier, defineImportAssertionAlias(context), meta);
      nextLoad = nextLoad.next;
    }

    const hookErrIdentifier = nextLoad.errIdentifier; // Retrieve the value after all settled
...
class Hook {
  ...
  constructor(fn, { type, url }, validateArgs) {
    console.log('new Hook:', { // FIXME: delete me
      fn,
      type,
      url,
    })
