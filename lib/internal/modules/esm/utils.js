...
async function initializeHooks() {
  ...
  const { CustomizedESMLoader } = require('internal/modules/esm/loader');
  const privateEsmLoader = new CustomizedESMLoader();
...
debug('importing custom loader', customLoaderPath);
    ...
    const keyedExportsSublist = await privateEsmLoader.import(
...
debug('completed importing custom hooks');
debug('instantiating Hooks{}');
  const { Hooks } = require('internal/modules/esm/hooks');
  const hooks = new Hooks(importedCustomLoaders);
debug('hooks instantiated')
debug('triggering preload')
  hooks.preload();
...
debug('importing module preloads', preloadModules)
    await privateEsmLoader.import(
      preloadModules,
      parentURL,
      kEmptyObject,
    );
