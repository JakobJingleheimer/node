...
debug('Hooks::resolve()', { originalSpecifier, parentURL, importAssertions })
...
debug('Hooks::resolve() resolution', resolution)
    if (typeof resolution?.then === 'function') {
debug('Hooks:resolve() resolution is thenable; thenning');
...
debug('HooksProxy::makeRequest()', type, ...args)
    if (!this.#isReady) {
debug('HooksProxy::makeRequest() checking if worker is ready')
      const { kIsOnline } = require('internal/worker');
      if (!this.worker[kIsOnline]) {
debug('HooksProxy::makeRequest() worker is NOT ready. sleeping until signaled');
        Atomics.wait(this.#lock, 0, 0); // ! Block the main thread until the worker is ready.
      }

      this.#isReady = true;
debug('HooksProxy::makeRequest() awoken: worker is now ready');
    }

    const { serialize } = require('v8');
debug('HooksProxy::makeRequest() emptying data vehicle');
...
debug('HooksProxy::makeRequest() sent request to worker. sleeping until notified');
    Atomics.wait(this.#lock, 0, 0); // Sleep until worker responds
debug('HooksProxy::makeRequest() awoken: received response from worker');
    const { deserialize } = require('v8');

    const response = deserialize(this.#requestResponseData);

debug('HooksProxy::makeRequest() response', type, '=>', response)
